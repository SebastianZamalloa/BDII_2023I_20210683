...
struct regist_direction
{
    ...
    regist_direction():id(0),bloq_num(0),sec_num(0),space(0){}
    regist_direction(int a,int b,int c,int d):id(a),bloq_num(b),sec_num(c),space(d){}
    
    regist_direction operator=(int response) const {
        if(response == 0)
            return {0,0,0,0};
        else
            return {id,bloq_num,sec_num,space};
    }

    bool operator==(const regist_direction& other) const {
        return id == other.id;
    }
...
std::ostream& operator<<(std::ostream& os, const regist_direction& myStruct) {
    os <<myStruct.id<<"#"<<myStruct.bloq_num<<"#"<<myStruct.sec_num<<"#"<<myStruct.space;
    return os;
}
